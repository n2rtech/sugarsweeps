            ...
            <div class="card-body">
                <ul class="list-group">
                    ...
                    @if($notification->type == 'credit-requested')
                        <li class="list-group-item"><a href="{{ route('admin.players.edit', $notification->user->id) }}">{{ $notification->user->name }}'s </a> {{ $notification->data->platform->platform }} account has been credited with ${{ $notification->data->amount }} credit. <span class="float-right badge badge-light">{{ Carbon\Carbon::parse($notification->created_at)->format('d-m-Y h:i:s') }}</span></li>
                    @endif

                    @if($notification->type == 'redeem-request')
                        <li class="list-group-item"><a href="{{ route('admin.players.edit', $notification->user->id) }}">{{ $notification->user->name }}</a> has requested to redeem @if($notification->data->redeem_full == "yes") all  @else worth ${{ $notification->data->amount }} @endif credits from {{ $notification->data->platform->platform }} account.<span class="float-right badge badge-light">{{ Carbon\Carbon::parse($notification->created_at)->format('d-m-Y h:i:s') }}</span></li>
                    @endif
