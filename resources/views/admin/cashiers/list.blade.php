            ...
            <div class="ms-panel-header">
                <div class="row">
                    ...
                    <div class="col-sm-6 text-right">
                        ...
                        <a href="{{ route('admin.cashiers.create') }}" class="btn btn-gradient-dark">Create</a>
                    </div>
                </div>
            </div>
                        ...
                        <form action="{{ route('admin.cashiers.index') }}">
                            <div class="form-row">
                                ...
                                <div class="col-xl-2 col-md-12 text-right">
                                    ...
                                    <a href="{{ route('admin.cashiers.index') }}" class="ms-btn-icon btn-gradient-warning form-control" type="submit"><i class="fas fa-undo"></i></a>
                                </div>
                            </div>
                        </form>
                    </div>
                            ...
                            <table class="table table-hover thead-dark">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><img src="{{ asset('assets/img/avatar.png') }}" style="width:50px; height:30px;">{{ $cashier->firstname }} {{ $cashier->lastname }}</td>
                                        <td>{{ $cashier->email }}</td>
                                        <td>{{ $cashier->phone }}</td>
                                        @if($cashier->status == 'inactive')
                                        <td><span class="badge badge-outline-danger">{{ ucfirst($cashier->status) }}</span>
                                        </td>
                                        @else
                                        <td><span class="badge badge-outline-light">{{ ucfirst($cashier->status) }}</span>
                                        </td>
                                        @endif
                                        <td>
                                            <a href="{{ route('admin.cashiers.edit', $cashier->id) }}" class="ms-btn-icon btn-secondary"><i class="fas fa-pencil-alt ms-text-success"></i> </a>
                                            <a href="#modal-delete" data-id="{{ $cashier->id }}" class="confirmDelete ms-btn-icon btn-danger"
