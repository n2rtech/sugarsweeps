            ...
            <div class="ms-panel-header">
                <div class="row">
                    ...
                    <div class="col-sm-6 text-right">
                        ...
                        <a href="{{ route('admin.vendors.create') }}" class="btn btn-gradient-dark">Create</a>
                    </div>
                </div>
            </div>
                        ...
                        <form action="{{ route('admin.vendors.index') }}">
                            <div class="form-row">
                                ...
                                <div class="col-xl-2 col-md-12 text-right">
                                    ...
                                    <a href="{{ route('admin.vendors.index') }}" class="ms-btn-icon btn-gradient-warning form-control" type="submit"><i class="fas fa-undo"></i></a>
                                </div>
                            </div>
                        </form>
                    </div>
                            ...
                            <table class="table table-hover thead-dark">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><img src="{{ asset('assets/img/avatar.png') }}" style="width:50px; height:30px;">{{ $vendor->firstname }} {{ $vendor->lastname }}</td>
                                        <td>{{ $vendor->email }}</td>
                                        <td>{{ $vendor->phone }}</td>
                                        @if($vendor->status == 'inactive')
                                        <td><span class="badge badge-outline-danger">{{ ucfirst($vendor->status) }}</span>
                                        </td>
                                        @else
                                        <td><span class="badge badge-outline-light">{{ ucfirst($vendor->status) }}</span>
                                        </td>
                                        @endif
                                        <td>
                                            <a href="{{ route('admin.vendors.edit', $vendor->id) }}"><i class="fas fa-pencil-alt ms-text-success"></i> </a>
                                            <a href="#modal-delete" data-id="{{ $vendor->id }}" class="confirmDelete"
...
<script>
    ...
    form_action = '{{ route("admin.vendors.destroy", ":id") }}';
